...
def read_json_corpus(json_path):
    ...
    for json_object in json_file['all_papers']:
        docno = json_object['docno']
        title = json_object['title'][0]
        paperAbstract = json_object['paperAbstract'][0]
        tokens = title.split(" ")
        for t in tokens:
            o.write(t.lower() + "\t" + str(docno) + "\n")
        tokens = paperAbstract.split(" ")
        for t in tokens:
            o.write(t.lower() + "\t" + str(docno) + "\n")
    o.close()
...
def read_corpus(dir):
    doc_id = 1
    o = open(dir + "/intermediate/output.tsv", "w", encoding="utf-8")
    for f in os.listdir(dir):
        if f.endswith(".txt"):
            f1 = open(dir + "/" + f, encoding="utf-8")
            for line in f1:
                tokens = line.split(" ")
                for t in tokens:
...
def construct_vector_from_postings(postings, terms):
    vector = []
    for p in postings:
        if p in terms:
            vector.append(1)
...
def rank_and_evaluate(corpus, query_file, qrel_file):
    # your code for ranking goes here
    # please feel free to add functions
    # or libraries of your choice
    # However, do not delete any of the
    # existing functions which may lead
    # to your code not running successfully
